...
final class SearchField: NSView {
    ...
    private func scheduleCommit() {
        ...
        let work = DispatchWorkItem { [weak self] in self?.commit(text) }
        debounce = work
...
extension SearchField: NSTextFieldDelegate {
    ...
    func controlTextDidBeginEditing(_ obj: Notification) { setFocused(true) }
...
private final class ClearButton: NSView {
    ...
    override init(frame frameRect: NSRect) {
        ...
        let img = NSImage(systemSymbolName: "xmark.circle.fill", accessibilityDescription: "Clear search")
        let cfg = NSImage.SymbolConfiguration(pointSize: 13, weight: .regular)
    ...
    override func mouseEntered(with event: NSEvent) { imageView.contentTintColor = Theme.text }
    override func mouseExited(with event: NSEvent)  { imageView.contentTintColor = Theme.textTertiary }
}
