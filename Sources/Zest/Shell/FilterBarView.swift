...
final class FilterBarView: NSView {
    ...
    init(coordinator: AppCoordinator) {
        ...
        NSLayoutConstraint.activate([
            ...
            countLabel.leadingAnchor.constraint(greaterThanOrEqualTo: scopeControl.trailingAnchor, constant: 10),
    ...
    private func updateCount() {
        ...
        let noun = coordinator.isSearchMode ? (n == 1 ? "result" : "results")
                                            : (n == 1 ? "item" : "items")
        ...
        s.append(NSAttributedString(string: " \(noun)", attributes: [
...
private final class ScopeControl: NSView {
    ...
    init(coordinator: AppCoordinator) {
        ...
        for (scope, title, symbol) in defs {
            let seg = Segment(title: title, symbol: symbol) { [weak self] in
    ...
    func refresh() {
        for seg in segments { seg.setSelected(seg.scope == coordinator.scope) }
    }
    ...
    final class Segment: NSView {
        ...
        init(title: String, symbol: String, onClick: @escaping () -> Void) {
            ...
            icon.image = NSImage(systemSymbolName: symbol, accessibilityDescription: title)?.withSymbolConfiguration(cfg)
...
private final class SortLabelButton: NSView {
    ...
    override init(frame frameRect: NSRect) {
        ...
        let cfg = NSImage.SymbolConfiguration(pointSize: 11, weight: .regular)
        icon.image = NSImage(systemSymbolName: "arrow.up.arrow.down", accessibilityDescription: "Sort")?.withSymbolConfiguration(cfg)
        icon.contentTintColor = Theme.textSecondary
        icon.imageScaling = .scaleProportionallyDown
        icon.translatesAutoresizingMaskIntoConstraints = false
