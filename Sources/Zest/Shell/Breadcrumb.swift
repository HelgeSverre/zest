...
final class Breadcrumb: NSView {
    ...
    func refresh() {
        ...
        for (i, seg) in segments.enumerated() {
            ...
            stack.addArrangedSubview(makeSegment(text: seg.name, path: seg.fullPath, style: isCurrent ? .current : .parent))
        }
    }

    private enum SegStyle { case home, parent, current }
    ...
    private func makeSegment(text: String, path: String, style: SegStyle) -> SegmentView {
        ...
        return SegmentView(text: text, font: font, color: color, navPath: path) { [weak self] target in
    ...
    private func makeRootToken() -> SegmentView {
        SegmentView(text: "/", font: .systemFont(ofSize: 12.5, weight: .regular),
                    color: Theme.textTertiary, navPath: "/") { [weak self] target in
            self?.coordinator.navigate(to: target)
        }
    }
    ...
    override func mouseDown(with event: NSEvent) {
        if editing { super.mouseDown(with: event); return }
...
private final class SegmentView: NSView {
    ...
    init(text: String, font: NSFont, color: NSColor, navPath: String, onClick: @escaping (String) -> Void) {
