...
fn parseKind(negated: bool, value: []const u8) ?FilterCriterion {
...
fn parseSizeFilter(value: []const u8) ?FilterCriterion {
    if (std.mem.indexOf(u8, value, "..")) |sep| {
        const lower = parseSize(value[0..sep]) catch return null;
        const upper = parseSize(value[sep + 2 ..]) catch return null;
        return .{ .size = .{ .op = .range, .value = lower, .value_upper = upper } };
    }

    if (value.len > 1) {
        if (value[0] == '>' and value[1] == '=') {
            const bytes = parseSize(value[2..]) catch return null;
        ...
        if (value[0] == '>') {
            const bytes = parseSize(value[1..]) catch return null;
    ...
    const bytes = parseSize(value) catch return null;
...
fn parseCategory(negated: bool, value: []const u8) ?FilterCriterion {
...
fn parsePath(negated: bool, value: []const u8) ?FilterCriterion {
    if (value.len == 0 or value.len > 255) return null;
    var f = FilterCriterion{ .path = .{ .negated = negated } };
...
test "parse mixed text and filters" {
    ...
    try std.testing.expectEqual(CompareOp.gt, result.filters_list[1].size.op);
