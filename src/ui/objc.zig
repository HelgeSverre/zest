...
// Re-exported from core/dispatch.zig so core-layer code doesn't depend on ui.
// ---------------------------------------------------------------------------

const dispatch_mod = @import("../core/dispatch.zig");

pub const dispatch_queue_t = dispatch_mod.dispatch_queue_t;
pub const dispatch_function_t = dispatch_mod.dispatch_function_t;

/// Returns the main dispatch queue (runs on the AppKit main thread).
pub fn dispatch_get_main_queue() dispatch_queue_t {
    return dispatch_mod.dispatch_get_main_queue();
}
...
pub fn dispatch_async_f(queue: dispatch_queue_t, context: ?*anyopaque, work: dispatch_function_t) void {
    dispatch_mod.dispatch_async_f(queue, context, work);
}
