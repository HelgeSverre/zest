> **Archived 2026-09-11.** Historical diagnosis and phase plan; most items are
> done. Live docs: `docs/ARCHITECTURE.md`, `docs/TESTING.md`, `docs/BENCHMARKS.md`.
...
- [x] C8. C ABI error reporting — `zest_query_cancellable` takes an
  `out_status` (`ZEST_QUERY_OK` / `ZEST_QUERY_ERROR` / `ZEST_QUERY_CANCELLED`,
  see docs/CAPI.md §2) so Swift can tell a cancelled query from a failed one.
...
- [x] E1. Saved-filters manager (`Shell/SavedFiltersDialog.swift`, persisted
  Swift-side in `filters.json` via `UserState`).
...
- [x] E1e. App bundle — shipped as `Zest.app` (`macos/Info.plist`,
  `scripts/package.sh`, `just app-package`; signed/notarized PKG via
  `just pkg-*`). Status-bar click-to-copy indexed count (spec §15.D) is in
  `StatusBarView.copyCount`.
...
- [ ] E7. Tabs and rename-in-place — Finder parity features, prioritize by
  usage. (Quick Look preview and one-window-per-`zest PATH` invocation are
  done.)

## Benchmarks

Moved to [docs/BENCHMARKS.md](../BENCHMARKS.md).
...
Still live: `docs/ARCHITECTURE.md` (accurate). The plans and specs under
`docs/superpowers/` are implemented and kept for history; the GUI-redesign
spec's remaining items are tracked above as E1c/E1d.
